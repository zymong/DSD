...
from .gaflow_core import GAFlowCore
...
class CamLiGA_Core(nn.Module):
    def __init__(self, cfgs):
        ...
        # Name is important!
        self.branch_2d = GAFlowCore(cfgs)
    ...
    def forward(self, image1, image2, pc1, pc2, camera_info):
        ...
        # 将点云投影到相机二维平面上，生成对应二维图像坐标
        uv1 = project_pc2image(xyz1, camera_info)
        uv2 = project_pc2image(xyz2, camera_info)
        # 用 (feat1_2d.shape[-1] - 1) / (sensor_w - 1) （水平方向上的比例）和 
        # (feat1_2d.shape[-2] - 1) / (sensor_h - 1) （垂直方向上的比例）
        # 分别对 uv1 和 uv2 进行相应的归一化处理。
        # 实现由图像坐标到像素坐标的转换
        ...
        if self.cfgs.gma:
            att_2d = self.branch_2d.att(x_2d)
        ...
        feat1_2d = self.branch_2d.fnet_aligner(feat1_2d.float())
        feat2_2d = self.branch_2d.fnet_aligner(feat2_2d.float())
        # GCL
        if self.cfgs.gcl:
            # feat1_2d = feat1_2d.float()
            # feat2_2d = feat2_2d.float()

            feat1_2d = self.branch_2d.gcl(feat1_2d)
            feat2_2d = self.branch_2d.gcl(feat2_2d)
        ...
        for it in range(n_iters):
            # 如果不是第一次迭代，就执行一些 detach 操作，即截断计算图，防止梯度反向传播到之前的迭代中。
            if it > 0:
            ...
            # correlation
            # 对网格坐标加上预测的 2D 光流，然后与第一张图像进行相关性计算，得到 2D 相关性。
            corr2d = self.branch_2d.correlation(grid_coords + flow_2d_pred)
            # 将 3D 点云坐标 xyz1 和 warp 后的点云坐标 xyzs2_warp 输入到模型中计算 3D 相关性。
            ...
            # motion features
            # 将 2D 光流和 2D 相关性输入到模型中，用于计算 2D 运动特征。
            motion_feat2d = self.branch_2d.motion_encoder(flow_2d_pred, corr2d)
            
            # 将输入的点云坐标、3D 场景流预测值和 3D 相关性输入到模型中，用于计算 3D 运动特征。
            # 其中 knn_indices 是邻域信息，用于计算局部运动特征。
            ...
            # GGAM
            if self.cfgs.ggam:
                motion_features_global_2d = self.branch_2d.aggregator(x_2d, motion_feat2d, it)
                inp_cat = torch.cat([x_2d, motion_feat2d, motion_features_global_2d], dim=1)
                # GRU
                h_2d = self.branch_2d.gru(torch.cat([h_2d, inp_cat], dim=1))
            # GMA
            elif self.cfgs.gma:
                motion_features_global_2d = self.branch_2d.aggregator(att_2d, motion_feat2d)
                inp_cat = torch.cat([x_2d, motion_feat2d, motion_features_global_2d], dim=1)
                # GRU
                h_2d = self.branch_2d.gru(torch.cat([h_2d, inp_cat], dim=1))
            else:
                inp_cat = torch.cat([x_2d, motion_feat2d], dim=1)
                h_2d = self.branch_2d.gru(torch.cat([h_2d, inp_cat], dim=1))
