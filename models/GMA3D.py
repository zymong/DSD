...
class Gma3D(nn.Module):
    ...
    def __init__(self, context_features_dim=128, motion_features_dim=128, gma_dim=128):
...
class GFE(nn.Module):
    def __init__(self, features_dim=128):
        ...
        self.sa = self_attention(channels=features_dim)
    ...
    def forward(self, features, xyz1=None):
        # b, c, n
        attention_matrix = self.sa(features, xyz1)
        # x_v = self.v_conv(motion_features)

        feature_enhanced = torch.bmm(features, attention_matrix)
