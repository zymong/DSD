...
def calc_sequence_loss_3d_addfeature(flow_preds, target, feature1_dense, feature2_dense, feature1_sparse, feature2_sparse, cfgs):
    """Sequence loss for Point-RAFT."""
    ...
    for i in range(n_preds):
        ...
        loss_flow = torch.pow(diff.abs().sum(dim=1) + 0.01, 0.4)[flow_mask].mean()

        weight = cfgs.gamma ** (n_preds - i - 1)
        total_loss += weight * loss_flow

    loss_feature1 = torch.linalg.norm(feature1_dense - feature1_sparse, dim=1)[flow_mask].mean()
    loss_feature2 = torch.linalg.norm(feature2_dense - feature2_sparse, dim=1)[flow_mask].mean()
    total_loss +=  beta * loss_feature1 + beta * loss_feature2
