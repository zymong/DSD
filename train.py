...
# python train.py trainset=kitti valset=kitti model=camliga ckpt.path=checkpoints/camliga_things150e.pt
# python train.py trainset=flyingthings3d_subset valset=flyingthings3d_subset model=camliga ckpt.path=outputs/camliga/2024-01-03/22-27-54/ckpts/epoch-145.pt ckpt.resume=true
# python train.py trainset=kitti valset=kitti model=camliga ckpt.path=outputs/camliga/2024-01-03/22-27-54/ckpts/epoch-145.pt
# python train.py trainset=kitti valset=kitti model=camliga ckpt.path=checkpoints/camligac_things145e_kitti790e.pt
# python train.py trainset=flyingthings3d_subset valset=flyingthings3d_subset model=camliga_l ckpt.path=outputs/camliga_l/2024-05-13/20-00-38/ckpts/epoch-077.pt ckpt.resume=true
# python train.py trainset=flyingthings3d_subset valset=flyingthings3d_subset model=camliraft_l ckpt.path=outputs/camliraft_l/2024-05-09/16-37-26/ckpts/epoch-100.pt ckpt.resume=true
# python train.py trainset=ft3do valset=ft3do model=gmsf
# python train.py trainset=flyingthings3d_subset valset=flyingthings3d_subset model=gmsf
...
class Trainer:
    def __init__(self, device: torch.device, cfgs: DictConfig):
        ...
        if self.cfgs.model.name == 'gmsf':
            ...
            #     print(k)
            # 加载一部分参数
            pretrained_path = "/mnt/save_home_all/home/wxy/code/GMSF/checkpoints/FTD_o.pth"
            # pretrained_path = "/home/wxy/code/camliga/outputs/gmsf/2024-12-17/13-15-06/ckpts/epoch-200.pt"
            pretrained_dict = torch.load(pretrained_path, map_location={'cuda:0': f'cuda:{self.device.index}'})
            # pretrained_dict = torch.load(pretrained_path, map_location=self.device)
            print('Load pretrained model from %s:' % pretrained_path)
            select = 'gmsf1'
            ...
            print("Pretrained only keys:", len(pretrained_keys - model_keys))
            print("Model only keys:", len(model_keys - pretrained_keys))
            ...
            # print(pretrained_dict)
            assert len(pretrained_dict) > 0, 'No valid parameters loaded from the pretrained model'
