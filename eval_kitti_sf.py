...
# python eval_kitti_sf.py testset=kittio model=gmsf ckpt.path=/home/wxy/code/camliga/checkpoints/gmsfgru8192.pt
# python eval_kitti_sf.py testset=kittis model=gmsf ckpt.path=/home/wxy/code/camliga/checkpoints/gmsfgru8192.pt
...
class Evaluator:
    ...
    def run(self):
        ...
        for inputs in tqdm(self.test_loader):
            ...
            for batch_id in range(size_of_batch(inputs)):
                ...
                if flow_3d_target.shape[0] > 3:
                    ...
                    flow_3d_mask = torch.ones(flow_3d_target.shape[1], dtype=torch.int64).cuda()
                    flow_3d_target = flow_3d_target
        ...
        logging.info('AccS: %.2f%%' % (metrics_3d['5cm'] / metrics_3d['counts'] * 100.0))
        logging.info('AccR: %.2f%%' % (metrics_3d['10cm'] / metrics_3d['counts'] * 100.0))
